...
public:
    ...
    board_support_ACImpl(Everest::ModuleAdapter* ev, const Everest::PtrContainer<ChargebyteTarragonDriver>& mod,
                         Conf& config) :
...
private:
    const Everest::PtrContainer<ChargebyteTarragonDriver>& mod;
    const Conf& config;
    ...
    std::shared_ptr<PowerSwitch> power_switch;
    std::shared_ptr<ControlPilot> control_pilot;
    std::thread charger_thread;
