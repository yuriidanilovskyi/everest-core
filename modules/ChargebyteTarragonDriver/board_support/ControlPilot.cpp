...
ControlPilot::ControlPilot(std::shared_ptr<ControlPilot_HAL> _control_pilot_hal,
                 std::shared_ptr<PowerSwitch> _powerSwitch) :
...
bool ControlPilot::readFromCar(CPState *cp) {
    ...
    if (simulation) {
        ...
        if (control_pilot_hal->readCPSignal()) {
            // EVLOG_info << "Reading real cp signal";
            cpLo = control_pilot_hal->getCPLo();
            cpHi = control_pilot_hal->getCPHi();
            // EVLOG_info << "cpLo: " << cpLo << " cpHi: " << cpHi;
...
int32_t ControlPilot::getMinPhaseCount() {
    return control_pilot_hal->getMinPhaseCount();
}

int32_t ControlPilot::getMaxPhaseCount() {
    return control_pilot_hal->getMaxPhaseCount();
}

int32_t ControlPilot::getMinCurrentA() {
    return control_pilot_hal->getMinCurrentA();
}

int32_t ControlPilot::getMaxCurrentA() {
    return control_pilot_hal->getMaxCurrentA();
}
...
void ControlPilot::startTimer(uint32_t msecs) {
    timerCountdown = std::chrono::milliseconds(msecs);
    timerTick = std::chrono::steady_clock::now();
}

bool ControlPilot::timerElapsed() {
    if (std::chrono::steady_clock::now() - timerTick > timerCountdown)
        return true;
    else
        return false;
}
