...
const char *debug_level_mqtt_string_map[DLOG_LEVEL_NUMLEVELS] = {
	"always",
	"error",
	"warning",
	"info",
...
// FIXME: inline?
void dlog_func(const dloglevel_t loglevel, const char *filename, const int linenumber, const char *functionname, const char *format, ...) {
	// fast exit
	...
	size_t offset = strftime(log_datetimestamp, sizeof(log_datetimestamp), "[%H:%M:%S", gmtime_r(&debug_tval.tv_sec, &tm));
	if (offset < 1) {
	...
	if (format) {
		...
		format_copy = (char *) calloc(1, formatlen + 1); // additional byte for terminating \0
	...
	switch(loglevel) {
		...
		case DLOG_LEVEL_TRACE:
			...
			break;
		default:
			EVLOG_critical << "Unkown log level";
			break;
	}
}
...
void dlog_level_set(const dloglevel_t loglevel) {
	...
	if (newloglevel != minloglevel_current) {
		...
		dlog(DLOG_LEVEL_ALWAYS, "switched log level from %d (\"%s\") to %d (\"%s\")",
			 minloglevel_old, debug_level_logstring_map[minloglevel_old], newloglevel, debug_level_logstring_map[newloglevel]);
	}
}
